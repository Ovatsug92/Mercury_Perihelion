...
def point_perihelion(d_d, phi_i, phi_f):
    if d_d == 0:
        ...
        nr = math.ceil(phi_i / (2 * math.pi))
        theta2pi = nr * (2 * math.pi)
        ax.plot(theta2pi, r_perihelion, '*', markersize=10, color=(0.3, 0.3, 0.3))
    else:
        # Newtonian perihelion
        nr = math.ceil(phi_i / (2 * math.pi))
        theta2pi = nr * (2 * math.pi)
        ...
        varphi2piprec = (nr - 1) * (2 * math.pi)
        varphi_prec = (1 / (1 - D)) * (np.arccos((1 / E) * ((r0 / r_perihelion) - 1)) + varphi2piprec)
        if varphi_prec < phi_i:
            nr = math.floor(phi_f / (2 * math.pi))
            varphi2piprec = (nr - 1) * (2 * math.pi)
            varphi_prec = (1 / (1 - D)) * (np.arccos((1 / E) * ((r0 / r_perihelion) - 1)) + varphi2piprec)
        ax.plot(varphi_prec, r_perihelion, '*', markersize=10, color=(0.6, 0.6, 0.6))


def graph_boolean(cartesian):
    """Graph boolean variable: If True, the plot is cartesian. If False, the plot is polar"""
    global fig, ax
    if not cartesian:
        # plotting the orbits
...
def cartesian_background(phi_i, phi_f):
    """Cartesian background: Main function for cartesian graphs. Plots the apsides,
...
def polar_background(phi_i, phi_f):
    """Polar background: Main function for polar graphs. Plots the apsides,
        formats and labels the axes."""
    graph_boolean(False)
    # apsides
    orbit_apsides(r_perihelion, phi_i, phi_f)  # Perihelion circle
...
def orbits(n_orbs):
    ...
    if save_graph == 's':
        for i in range(n_orbs):
            polar_background(0, 2 * math.pi)
            # Newtonian orbit
            thetan = np.arange(0, 2 * math.pi, 0.01)
            rn = (h ** 2 / (G * M)) * (1 / (1 + E * np.cos(thetan)))
            ax.plot(thetan, rn, 'g')
            # Relativistic orbit
            thetar = np.arange(i * thetapi4right + 0.1, i * thetapi4right + 2 * math.pi, 0.1)
            rprec = (h ** 2 / (G * M)) * (1 / (1 + E * np.cos(thetar * (1 - D))))
            ax.plot(thetar, rprec, 'b.', markersize=3)
            # Relativistic perihelion
            ax.plot(i * thetapi4right, r_perihelion, '*', markersize=8, color=(0.6, 0.6, 0.6))
            # Newtonian perihelion
    ...
    for it in range(3):
        for j in range(3):
            ...
            # Circles
            rpp = r_perihelion * (theta ** 0)  # Perihelion Circle
            raphp = r_aphelion * (theta ** 0)  # Aphelion Circle
            ...
            if it == 0:
                ...
                # Relativistic perihelion
                axxs[it, j].plot((it + j) * thetapi4right, r_perihelion, '*', markersize=8, color=(0.6, 0.6, 0.6))
            elif it == 1:
                # Title
                ...
                axxs[it, j].plot(theta, rperprec, 'b.', markersize=1.5)
                # Relativistic perihelion
                axxs[it, j].plot((it + 2 + j) * thetapi4right, r_perihelion, '*', markersize=8, color=(0.6, 0.6, 0.6))
            elif it == 2:
                # Title
                ...
                axxs[it, j].plot(theta, rperprec, 'b.', markersize=1.5)
                # Relativistic perihelion
                axxs[it, j].plot((it + 4 + j) * thetapi4right, r_perihelion, '*', markersize=8, color=(0.6, 0.6, 0.6))
            # Newtonian perihelion
            axxs[it, j].plot(0, r_perihelion, '*', markersize=8, color=(0.3, 0.3, 0.3))
    if saveorbsall == 's':
        print('1. Pdf: Para guardar en formato ".pdf" escriba 1 y presione enter.')
        print('2. Png: Para guardar en formato ".png" escriba 2 y presione enter.')
        time.sleep(1)
...
def constants():
    ...
    data_m = [m, Td, T, r_perihelion, r_aphelion, ra, E]
...
def returning():
    ...
    global run_program, cartesian_graph_boolean
    plt.close()
...
def returning_sub():
    plt.close()
    global run_subprogram, cartesian_graph_boolean
...
while run_program:
    ...
    if int(c_main) == 3:
        ...
        while run_subprogram:
            ...
            if int(c_sub) == 4:
                sub_sub_menu()
                save_simulation(D, n4 * thetapi4right, n4 * thetapi4right + 2 * np.pi, c_sub_sub)
                returning_sub()
    if int(c_main) == 4:
        orbits(8)
        time.sleep(1)
