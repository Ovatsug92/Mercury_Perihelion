...
def point_perihelion(d_d, phi_i, phi_f):
    ...
    if d_d == 0:
        ...
        phi_2pi_relativistic = (n_2pi - 1) * (2 * math.pi)
        phi_perihelion = (1 / (1 - D)) * (np.arccos((1 / E) * ((r0 / r_perihelion) - 1)) + phi_2pi_relativistic)
        if phi_perihelion < phi_i:
            n_2pi = math.floor(phi_f / (2 * math.pi))
            phi_2pi_relativistic = (n_2pi - 1) * (2 * math.pi)
            phi_perihelion = (1 / (1 - D)) * (np.arccos((1 / E) * ((r0 / r_perihelion) - 1)) + phi_2pi_relativistic)
    ...
    for it in range(3):
        for j in range(3):
            ...
            if it == 0:
                ...
                # Precessed orbit
                phi = np.arange((it + j) * phi_pi4_right, (it + j) * phi_pi4_right + 2 * math.pi, 0.1)
                r_relativistic = (h ** 2 / (G * M)) * (1 / (1 + E * np.cos(phi * (1 - D))))
                ax_s[it, j].plot(phi, r_relativistic, 'b.', markersize=1.5)
                # Relativistic perihelion
                ax_s[it, j].plot((it + j) * phi_pi4_right, r_perihelion, '*', markersize=8, color=(0.6, 0.6, 0.6))
            elif it == 1:
                # Title
                ax_s[it, j].set_title(f'Precesión {it + 2 + j} veces ' r'$\frac{\pi}{4}$')
                ax_s[it, j].title.set_size(8)
                # Precessed orbit
                phi = np.arange((it + 2 + j) * phi_pi4_right, (it + 2 + j) * phi_pi4_right + 2 * math.pi, 0.1)
                r_relativistic = (h ** 2 / (G * M)) * (1 / (1 + E * np.cos(phi * (1 - D))))
                ax_s[it, j].plot(phi, r_relativistic, 'b.', markersize=1.5)
                # Relativistic perihelion
                ax_s[it, j].plot((it + 2 + j) * phi_pi4_right, r_perihelion, '*', markersize=8, color=(0.6, 0.6, 0.6))
            elif it == 2:
                # Title
                ax_s[it, j].set_title(f'Precesión {it + 4 + j} veces ' r'$\frac{\pi}{4}$')
                ax_s[it, j].title.set_size(8)
                # Precessed orbit
                phi = np.arange((it + 4 + j) * phi_pi4_right, (it + 4 + j) * phi_pi4_right + 2 * math.pi, 0.1)
                r_relativistic = (h ** 2 / (G * M)) * (1 / (1 + E * np.cos(phi * (1 - D))))
                ax_s[it, j].plot(phi, r_relativistic, 'b.', markersize=1.5)
                # Relativistic perihelion
                ax_s[it, j].plot((it + 4 + j) * phi_pi4_right, r_perihelion, '*', markersize=8, color=(0.6, 0.6, 0.6))
            # Newtonian perihelion
...
wp = (6 * math.pi * G * M) / (r_a * c ** 2 * (1 - E ** 2))  # 5.018896261130917e-07
...
while run_program:
    ...
    if int(c_main) == 3:
        ...
        while run_subprogram:
            ...
            if c_sub == 1:
                print('Luego de obervar la gráfica, por favor cerrar la ventana.')
                cartesian_graph(D, n4 * phi_pi4_right, n4 * phi_pi4_right + 2 * np.pi)
                returning_sub()
            if c_sub == 2:
                print('Luego de obervar la gráfica, por favor cerrar la ventana.')
                polar_graph(D, n4 * phi_pi4_right, n4 * phi_pi4_right + 2 * np.pi)
                returning_sub()
            if c_sub == 3:
                print('Luego de obervar la simulación, por favor cerrar la ventana.')
                simulation(D, n4 * phi_pi4_right, n4 * phi_pi4_right + 2 * np.pi)
                returning_sub()
            if int(c_sub) == 4:
                sub_sub_menu()
                save_simulation(D, n4 * phi_pi4_right, n4 * phi_pi4_right + 2 * np.pi, c_sub_sub)
                returning_sub()
    if int(c_main) == 4:
        orbits(8)
        time.sleep(1)
