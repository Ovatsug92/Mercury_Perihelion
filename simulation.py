...
def orbit_sim(nsim):
    ...
    # for theta in theta1:
    theta = nsim * thetapi4right
    while theta < nsim * thetapi4right + 2 * math.pi:
        rperprec = (h ** 2 / (G * M)) * (1 / (1 + E * np.cos(theta * (1 - D))))
        plt.xticks(rs, rad)  # less labels for radius and change degrees to  multipli of pi/4 radians
        ax.set_rlabel_position(0)  # Move radial labels away from plotted line
...
def main_menu():
    ...
    print('2. Realizar la simulación de la órbita de Mercurio precesada comprendida entre dos ángulos  de pi/4 '
          'radianes.')
    print('3. Realizar la simulación de la órbita de Mercurio precesada en un múltiplo de pi/4 radianes.')
    print('4. Mostrar las gráficas de las órbitas precesadas en múltiplos de pi/4 radianes.')
    print('5. Mostrar la tabla de datos usados en este programa.')
...
def sub_menu():
    ...
    print('Escoger una opción que le gustaría realizar de la siguiente lista:\n')
    time.sleep(1.5)
    print('1. Mostrar la gráfica de la simulación.')
    print('2. Mostar la simulación.')
    print('3. Guardar la simulación.')
    time.sleep(3)
    csub = int(input('Escribir el número de la opción elegida y presionar enter, por favor:\n'))
    while (csub < 1) or (csub > 6):
        print(f'El número correspondiente a la opción debe ser entero y debe estrar entre  entre 0 y 9\n')
        time.sleep(1)
        csub = int(input('Escribir el número de la opción elegida y presionar enter, por favor:\n'))
...
def sub_sub_menu():
    ...
    time.sleep(1)
    csubsub = input('Escriba su opción, por favor:\n')
    while (csub < 1) or (csub > 3):
...
while run_program:
    ...
    #         sys.exit()
    # elif int(c0) == 2:
    #     print('Luego de observar la gráfica, por favor cerrar la ventana')
